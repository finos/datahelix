...
class OfTypeConstraintFactoryTest {
    ...
    void returnsNullWhenPassedDecimalLowerCase() {
        Optional<Constraint> constraint = OfTypeConstraintFactory.create(field, SpecificFieldType.DECIMAL);
        assertFalse(constraint.isPresent());
    }

    @Test
    void returnsGranularToOneWhenPassedInteger() {
        Optional<Constraint> constraint = OfTypeConstraintFactory.create(field,SpecificFieldType.INTEGER);
    ...
    void returnsStandardRICConstraintWhenPassedUpperCaseRIC() {
        Optional<Constraint> constraint = OfTypeConstraintFactory.create(field,SpecificFieldType.RIC);
        assertTrue(constraint.isPresent());
    ...
    void returnsInSetConstraintWhenPassedLowerCaseFullName() {
        Optional<Constraint> constraint = OfTypeConstraintFactory.create(field,SpecificFieldType.FULL_NAME);
