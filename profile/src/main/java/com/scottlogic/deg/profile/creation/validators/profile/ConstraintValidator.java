...
public abstract class ConstraintValidator<T extends ConstraintDTO> implements Validator<T>
{
    ...
    protected static ValidationResult validateConstraint(ConstraintDTO constraint, String rule, List<FieldDTO> fields)
    {
        ...
        {
            case EQUAL_TO:
                return new EqualToConstraintValidator(rule, fields).validate((EqualToConstraintDTO) constraint);
            case IN_SET:
                return new InSetConstraintValidator(rule, fields).validate((InSetConstraintDTO) constraint);
            case IN_MAP:
                return new InMapConstraintValidator(rule, fields).validate((InMapConstraintDTO) constraint);
            case IS_NULL:
                return new IsNullConstraintValidator(rule, fields).validate((IsNullConstraintDTO)constraint);
            case GRANULAR_TO:
                return new GranularToConstraintValidator(rule, fields).validate((GranularToConstraintDTO) constraint);
            case MATCHES_REGEX:
            case CONTAINS_REGEX:
                return new RegexConstraintValidator(rule, fields).validate((RegexConstraintDTO) constraint);
            case EQUAL_TO_FIELD:
            ...
            case BEFORE_OR_AT_FIELD:
                return new RelationalConstraintValidator<>(rule, fields).validate((RelationalConstraintDTO) constraint);
            case OF_LENGTH:
            case LONGER_THAN:
            case SHORTER_THAN:
                return new NumericConstraintValidator(rule, fields, FieldType.STRING).validate((NumericConstraintDTO) constraint);
            case GREATER_THAN:
            case GREATER_THAN_OR_EQUAL_TO:
            case LESS_THAN:
            case LESS_THAN_OR_EQUAL_TO:
                return new NumericConstraintValidator(rule, fields, FieldType.NUMERIC).validate((NumericConstraintDTO) constraint);
            case AFTER:
            case AFTER_OR_AT:
            case BEFORE:
            case BEFORE_OR_AT:
                return new TemporalConstraintValidator(rule, fields).validate((TemporalConstraintDTO) constraint);
            case NOT:
                return new NotConstraintValidator(rule, fields).validate((NotConstraintDTO) constraint);
            case ANY_OF:
                return new AnyOfConstraintValidator(rule, fields).validate((AnyOfConstraintDTO) constraint);
            case ALL_OF:
                return new AllOfConstraintValidator(rule, fields).validate((AllOfConstraintDTO) constraint);
            case IF:
                return new ConditionalConstraintValidator(rule, fields).validate((ConditionalConstraintDTO) constraint);
            default:
                throw new IllegalStateException("Unexpected constraint type: " + constraint.getType());
        }
    }
