...
import com.scottlogic.deg.classifier.{EmailType, SemanticType}
...
object EmailClassifier extends Classifier {
  ...
  override val semanticType: SemanticType = EmailType
  override def matches(input: String): Boolean = input.matches(emailRegex)
}
