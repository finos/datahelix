...
public class ContradictoryConstraintNode implements ConstraintNode, ContradictoryNode{
    private final ConstraintNode underlying;

    public ContradictoryConstraintNode(ConstraintNode underlying) {
    ...
    public ConstraintNode removeDecisions(Collection<DecisionNode> decisionsToRemove) {
        return new ContradictoryConstraintNode(underlying.removeDecisions(decisionsToRemove));
    }
    ...
    public ConstraintNode addAtomicConstraints(Collection<IConstraint> constraints) {
        return new ContradictoryConstraintNode(underlying.addAtomicConstraints(constraints));
    }

    @Override
    public ConstraintNode addDecisions(Collection<DecisionNode> decisions) {
        return new ContradictoryConstraintNode(underlying.addDecisions(decisions));
    }

    @Override
    public ConstraintNode setDecisions(Collection<DecisionNode> decisions) {
        return new ContradictoryConstraintNode(underlying.setDecisions(decisions));
    }

    @Override
    public boolean equals(Object o) {
        if (o instanceof ContradictoryConstraintNode)
            o = ((ContradictoryConstraintNode)o).underlying;
