...
class FieldSpecValueGeneratorTests {
    ...
    void generate_fieldSpecMustContainRestrictionWithValuesEmittedByOtherSource_returnsUniqueValues() {
        FieldSpec fieldSpec = FieldSpec.Empty
            ...
            .withMustContainRestriction(new MustContainRestriction(
    ...
    void generate_fieldSpecMustContainRestrictionWithValuesNotEmittedByOtherSource_returnsBothSets() {
        ...
        Assert.assertThat(valuesForFirstField, containsInAnyOrder(11, 12, 13, 14, 15, 16, 17, 18, 19, 25));
    }

    @Test
    void generate_fieldSpecMustContainRestrictionWithValuesBothEmittedAndNotEmittedByOtherSource_returnsUniqueValuesFromBoth() {
        FieldSpec fieldSpec = FieldSpec.Empty
            .withNumericRestrictions(
                new NumericRestrictions() {{
                    min = new NumericLimit<>(new BigDecimal(10), false);
            ...
            .withMustContainRestriction(new MustContainRestriction(
                Collections.singleton(FieldSpec.Empty
                    .withSetRestrictions(SetRestrictions.fromWhitelist(new HashSet<>(Arrays.asList(15, 25))), fieldSpecSource)
                    .withNullRestrictions(new NullRestrictions(Nullness.MUST_NOT_BE_NULL), fieldSpecSource)
                )
            ));
        GenerationConfig generationConfig = new GenerationConfig(
