...
class DecisionTreeWalkerTests {
    ...
    void test() {
        ...
        final ProfileDecisionTreeCollection decisionTreeCollection =
            new ProfileDecisionTreeCollection(
                dummyRules.stream().collect(
                    Collectors.toMap(
                        Function.identity(),
                        dTreeGenerator::generateTreeFor)));

        final List<RowSpec> rowSpecs = dTreeWalker
            .walk(decisionTreeCollection, fields)
