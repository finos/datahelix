Feature: User can specify that a datetime date is after, but not equal to, a specified datetime
  ...
  Scenario Outline: 'After' invalid datetime fails with error
    ...
    Then the profile is invalid because "Date string '\d{4}-\d{2}-\d{2}T\d{2}:00:00.000Z' must be in ISO-8601 format: yyyy-MM-ddTHH:mm:ss.SSS\[Z\] between \(inclusive\) 0001-01-01T00:00:00.000Z and 9999-12-31T23:59:59.999Z"
  ...
  Scenario: 'After' non-existent leap year date fails with error
    ...
    Then the profile is invalid because "Date string '2019-02-29T00:00:00.000Z' must be in ISO-8601 format: yyyy-MM-ddTHH:mm:ss.SSS\[Z\] between \(inclusive\) 0001-01-01T00:00:00.000Z and 9999-12-31T23:59:59.999Z"
    And no data is created
...
### beforeOrAt ###
  ...
  Scenario: Running a 'after' request that specifies the highest valid system date should be unsuccessful
    ...
    Then the profile is invalid because "Date string '10000-01-01T00:00:00.000Z' must be in ISO-8601 format: yyyy-MM-ddTHH:mm:ss.SSS\[Z\] between \(inclusive\) 0001-01-01T00:00:00.000Z and 9999-12-31T23:59:59.999Z"
