...
Scenario: Running an 'inSet' request that includes an invalid time value should fail with an error message
  Given there is a field foo
    And foo is in set:
      ...
      | 2017-12-31T23:59:59.999 |
  Then the profile is invalid because "Date string '2017-12-31T40:59:59.999' must be in ISO-8601 format: yyyy-MM-ddTHH:mm:ss.SSS between \(inclusive\) 0001-01-01T00:00:00.000 and 9999-12-31T23:59:59.999"
    And no data is created
...
Scenario Outline: Not 'inSet' type values with non-contradicting 'ofType' are successful
  ...
  Examples:
      ...
      | 1                       | "datetime" | 2019-01-01T00:00:00.000 |
      | "a"                     | "datetime" | 2019-01-01T00:00:00.000 |
...
Scenario Outline: : 'InSet' equal to a type value and with not 'ofType' for the same type should emit null
  ...
  Examples:
      ...
      | "a"                     | "string"   |
      | 1                       | "decimal"  |
      | 2010-01-01T00:00:00.000 | "datetime" |
