...
Scenario Outline: Running an 'ofLength' request that includes a value that is not a valid number should fail with an error
     Given foo is of length <length>
       And foo is in set:
         | "a" |
     Then the profile is invalid because "(Couldn't recognise 'value' property, it must be a Number but was a String with value `.*`, field: foo)|(Cannot create an StringHasLengthConstraint for field 'foo' with a a negative length., field: foo)|(ofLength constraint must have a operand/value >= 0, currently is -?\d+ for field \[foo\])"
